...
export async function currentUser(cookie?: string) {
  ...
  if (parts.length !== 3 || Number(parts[1]) < Date.now() || !timingSafeEqual(Buffer.from(parts[2]), Buffer.from(sign(`${parts[0]}.${parts[1]}`)))) return null;
  const rows = await query<{ id: string; email: string; name: string; role: "ADMIN" | "TRAINER" | "APPROVER"; signature_image?: string }>("SELECT id, email, name, role, signature_image FROM users WHERE id = $1", [parts[0]]);
  const user = rows[0];
...
export async function ensureUser(email: string, password: string) {
  const rows = await query<{ id: string; email: string; name: string; role: "ADMIN" | "TRAINER" | "APPROVER"; password_hash: string | null }>("SELECT id, email, name, role, password_hash FROM users WHERE email = $1", [email]);
  ...
  const id = randomUUID();
  await query("INSERT INTO users (id, email, name, password_hash, role) VALUES ($1, $2, $3, $4, 'ADMIN')", [id, email, email.split("@")[0], hashPassword(password)]);
