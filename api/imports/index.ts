...
export default async function handler(req: Request, res: Response) {
  ...
  if (!input?.templateId || !input.rows?.length || input.rows.some((row) => !row.recipient_name)) return res.status(400).json({ error: 'An active template and valid rows are required.' });
  try {
    ...
    const documents = await transaction(async (connection) => {
      await connection.query("INSERT INTO import_batches (id, template_id, file_name, total_rows, valid_rows, invalid_rows, status, uploaded_by, submitted_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 0, 'COMPLETED', ?, ?, ?, ?)", [batchId, input.templateId, input.fileName || 'import.xlsx', input.rows!.length, input.rows!.length, input.uploadedBy || null, timestamp, timestamp, timestamp]);
      const created = [];
      for (let index = 0; index < input.rows!.length; index += 1) {
        ...
        if (hasQr && !qr) throw new Error('The QR encryption service did not return a valid QR URL.');
