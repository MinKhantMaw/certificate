...
export function ImportExcel() {
  ...
  return <div className="space-y-6">
    ...
    {step === "UPLOAD" && <div className="rounded-xl border border-slate-200 bg-white p-12 text-center shadow-sm"><Upload className="mx-auto text-[#0054a6]" size={42} /><h3 className="mt-4 text-lg font-semibold text-slate-950">Upload Excel roster</h3><p className="mx-auto mt-2 max-w-lg text-sm text-slate-500">Required columns are varied depending on the selected template.</p><label className="mt-6 inline-flex cursor-pointer items-center gap-2 rounded-lg bg-[#0054a6] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#0054a6]"><Upload size={17} /> Select Excel file<input type="file" accept=".xlsx,.xls" disabled={!template} className="hidden" onChange={(event: ChangeEvent<HTMLInputElement>) => event.target.files?.[0] && upload(event.target.files[0])} /></label></div>}
