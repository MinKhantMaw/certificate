...
import { ArrowLeft, Download, ShieldAlert, ExternalLink, Printer } from 'lucide-react';
  ...
  if (!cert) {
    return (
      <div className="p-8 text-center text-gray-500">
        ...
        <br />
        <Link to="/certificates" className="text-blue-600 mt-4 inline-block hover:underline">Back to List</Link>
      </div>
    );
  }

  const verifyUrl = cert.verificationUrl || `${window.location.origin}/verify/${cert.verificationToken}`;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-xl shadow-sm border border-gray-200 print:hidden">
        <Link to="/certificates" className="flex items-center text-gray-600 hover:text-gray-900">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 print:hidden">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <h3 className="font-semibold text-gray-900 mb-4 border-b pb-2">Recipient Information</h3>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <h3 className="font-semibold text-gray-900 mb-4 border-b pb-2">System Information</h3>
          <div className="space-y-3">
            <div className="flex justify-between">
              <span className="text-gray-500">Status</span>
              <span className={`font-medium ${cert.status === 'VALID' ? 'text-green-600' : 'text-red-600'}`}>
                {cert.status}
              </span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-mono text-xs text-gray-600 break-all max-w-[200px] text-right">{cert.verificationToken}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-100 p-8 rounded-xl border border-gray-200 flex justify-center overflow-x-auto print:bg-white print:p-0 print:border-none print:m-0 print:block">
        <div className="print-container transform origin-top-left md:scale-100 sm:scale-75 scale-50 print:scale-100">
          <CertificatePreview certificate={cert} baseUrl={window.location.origin} />
        </div>
      </div>
