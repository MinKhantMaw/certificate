...
const documents: Document[] = [
  ...
  },
  {
    ...
    status: "VALID",
    documentTemplateId: "template-a",
    dynamicData: { recipient_name: "Zoe Smith" },
    createdAt: "",
  },
  {
    id: "DOC-3",
    ...
    dynamicData: { course_name: "Different template" },
    createdAt: "",
  },
];

const { updateDocumentStatus, deleteDocument } = vi.hoisted(() => ({
...
describe("DocumentList", () => {
  beforeEach(() => {
    ...
    act(() =>
      root.render(
        createElement(BrowserRouter, null, createElement(DocumentList)),
      ),
    );
  });
  ...
  it("filters by template and renders unique dynamic columns without IDs", async () => {
    await vi.waitFor(() =>
      expect(
        window.document.querySelector("select")?.querySelectorAll("option"),
      ).toHaveLength(3),
    );
  ...
  it("searches and sorts using visible dynamic values", async () => {
    ...
    const recipientHeader = [
      ...window.document.querySelectorAll("th button"),
    ].find((button) =>
      button.textContent?.includes("Recipient Name"),
  ...
  it("confirms document deletion before removing a row and reports failures", async () => {
    ...
    deleteDocument.mockImplementationOnce(() => {
  ...
  it("browses the filtered certificates in slide view", async () => {
    await vi.waitFor(() =>
      expect(
        window.document.querySelector("select")?.querySelectorAll("option"),
      ).toHaveLength(3),
    ...
    await act(async () =>
      [...window.document.querySelectorAll("button")]
        .find((button) => button.textContent?.includes("Slide view"))
        ?.click(),
    );

    expect(window.document.body.textContent).toContain("Certificate 1 of 2");
    expect(window.document.body.textContent).toContain("Fallback Name");
    await act(async () =>
      [...window.document.querySelectorAll("button")]
        .find((button) => button.textContent?.includes("Next"))
        ?.click(),
    );
    expect(window.document.body.textContent).toContain("Certificate 2 of 2");
    expect(window.document.body.textContent).toContain("Fallback Name");
  });
});
